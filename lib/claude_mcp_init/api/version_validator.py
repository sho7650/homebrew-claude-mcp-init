...
class VersionValidator:
    ...
    def check_version_consistency(self) -> Dict[str, Any]:
        ...
        current_version = (
            primary_version
            or versions.get("build_version")
            or versions.get("version_file")
        )
    ...
    def _find_discrepancies_git_based(
        self, versions: Dict[str, Optional[str]], primary_version: Optional[str]
    ) -> List[str]:
        """
        ...
        if not primary_version:
            if not versions.get("build_version"):
                discrepancies.append(
                    "No git tag or build version found - version source unavailable"
                )
            return discrepancies

        # Check formula version for releases (should match git tag for tagged releases)
        formula_version = versions.get("formula")
        if formula_version and not self._versions_compatible(
            primary_version, formula_version
        ):
            discrepancies.append(
        ...
        if build_version and not self._versions_compatible(
            primary_version, build_version
        ):
            # This is often expected in development - not necessarily an error
            if not primary_version.endswith("-dev") and "dev" not in primary_version:
                discrepancies.append(
