...
class FormulaUpdater:
    ...
    def update_formula_version(self, version: str, sha256: Optional[str] = None) -> bool:
        """
    ...
    def download_release_tarball(self, version: str, output_path: Optional[Path] = None) -> Path:
        """
        ...
        if output_path is None:
            output_path = Path(tempfile.gettempdir()) / f"{self.formula_name}-{version}.tar.gz"
    ...
    def generate_formula_from_template(self, version: str, description: str = None) -> str:
        """
  ...
  test do
    ...
    def create_release_notes(self, version: str, previous_version: Optional[str] = None) -> str:
        """
        ...
        notes.append("```bash")
        notes.append("brew tap {}/{}".format(
            self.github_repo.split('/')[0],
            "homebrew-" + self.formula_name
        ))
