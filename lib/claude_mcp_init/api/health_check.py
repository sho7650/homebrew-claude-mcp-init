...
class HealthChecker:
    ...
    def check_dependencies(self) -> Dict[str, Any]:
        ...
        if python_version:
            ...
            if major < 3 or (major == 3 and minor < 11):
                results["status"] = "unhealthy"
                results["required"]["python3"]["note"] = f"Python 3.11+ required, found {major}.{minor}"
    ...
    def check_configuration(self) -> Dict[str, Any]:
        ...
        for file_path, required in config_files:
            ...
            results["files"][file_path] = {
                "exists": exists,
                "required": required,
                "readable": full_path.is_file() and os.access(full_path, os.R_OK) if exists else False
            }
    ...
    def generate_report(self) -> str:
        ...
        disk = health["checks"]["disk_space"]
        report.append("\n## System Resources")
        report.append(f"- **Disk Space**: {disk['free_gb']}GB free of {disk['total_gb']}GB ({disk['used_percent']}% used)")
