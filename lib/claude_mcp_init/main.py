...
def create_cli():
    ...
    @click.option(
        ...
        "--in-place",
        is_flag=True,
        help="Create configuration in current directory",
    )
    @click.option(
        "--mcp", help="Comma-separated list of MCP modules to enable (serena,cipher)"
    )
    @click.argument("project_name", required=False)
    @click.argument("language", required=False)
    @click.pass_context
    def cli(
        ctx, version, help_modules, in_place, mcp, project_name, language, **kwargs
    ):
        """
        ...
        if not validate_project_name(project_name):
            click.echo(
                format_error(
                    "Invalid project name. Must start with alphanumeric character and "
                    "contain only letters, numbers, dots, hyphens, and underscores (max 100 chars)."
                )
            )
            sys.exit(1)
        ...
        if in_place:
            ...
            if _check_in_place_safety(context.project_path):
                click.echo(
                    format_error(
                        "Cannot use in-place mode: directory appears to contain important files"
                    )
                )
                sys.exit(1)
        else:
...
def _show_available_modules(plugin_manager: PluginManager):
    ...
    for plugin_name, plugin in plugin_manager.plugins.items():
        ...
        click.echo(
            f"  {plugin_name:12} - {metadata.get('description', 'No description')}"
        )
...
def _validate_plugin_requirements(context: MCPInitContext, modules: List[str]):
    ...
    for module_name in modules:
        ...
        if not valid:
            click.echo(
                format_error(f"Module {module_name} requirements not met: {error}")
            )
            sys.exit(1)
...
def _generate_configuration(context: MCPInitContext, modules: List[str]):
    ...
    click.echo(
        format_info(f"Setting up MCP configuration for '{config['project_name']}'...")
    )
    ...
    for module_name, plugin in active_plugins.items():
        try:
            ...
            click.echo(
                format_error(f"Failed to generate config for {module_name}: {e}")
            )
            sys.exit(1)
    ...
    click.echo(
        format_success(
            f"✅ MCP configuration complete! Project ready at {project_path}"
        )
    )


def _generate_mcp_json(
    project_path: Path, config: Dict[str, Any], plugins: Dict[str, Any]
):
...
def _generate_env_file(
    ...
    if env_vars:
        ...
        click.echo(
            format_warning(
                "⚠️  Keep .env file secure and don't commit it to version control"
            )
        )
...
def _generate_setup_instructions(project_path: Path, plugins: Dict[str, Any]):
    ...
    with instructions_file.open("w") as f:
        ...
        for module_name, plugin in plugins.items():
            metadata = plugin.metadata
            f.write(
                f"- **{module_name}**: {metadata.get('description', 'No description')}\n"
            )
...
def status(format):
    ...
    if format == "json":
        ...
        click.echo(
            f"Overall Status: "
            f"{click.style(status_info['status'].upper(), fg=status_color, bold=True)}"
        )
        click.echo(f"API Version: {status_info['api_version']}")
        click.echo(
            f"Current Version: {status_info['components']['version_validator']['current_version']}"
        )
        ...
        for name, info in status_info["components"].items():
            status_icon = {"healthy": "✅", "warning": "⚠️", "unhealthy": "❌"}.get(
                info["status"], "❓"
            )
...
def diagnose(format):
    ...
    if format == "json":
        ...
        if warnings > 0:
            click.echo(
                f"Warnings: {click.style(str(warnings), fg='yellow', bold=True)}"
            )
        ...
        if diagnosis["system_issues"]:
            ...
            for issue in diagnosis["system_issues"]:
                ...
                click.echo(
                    f"  {severity_icon} {click.style(issue['issue'], fg=severity_color)}"
                )
...
def prepare_release(version, dry_run):
    ...
    if dry_run:
        click.echo(
            f"🔍 {click.style('DRY RUN: Preparing release for version', bold=True)} {version}"
        )
        # Validation only
        validation = api_client.validate_system()
        if validation["valid"]:
    ...
    click.echo(
        f"🚀 {click.style('Preparing release for version', bold=True)} {version}"
    )
    ...
    if result["ready"]:
        click.echo(
            f"✅ {click.style('Release preparation complete!', fg='green', bold=True)}"
        )
        ...
        click.echo(
            f"❌ {click.style('Release preparation failed:', fg='red', bold=True)}"
        )
...
def version_info():
    ...
    for source, version in version_info["versions"].items():
        ...
        click.echo(
            f"  {status_icon} {source.replace('_', ' ').title()}: {display_version}"
        )
