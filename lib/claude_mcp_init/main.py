...
@click.pass_context
def cli(ctx, version, help_modules, in_place, mcp, project_name, language):
    """
    ...
    _configure_plugins(ctx, context, modules)
...
def _configure_plugins(ctx, context: MCPInitContext, modules: List[str]):
    """Configure plugins with dynamic CLI options"""
    # Add plugin-specific options to the CLI dynamically
    for module_name in modules:
        ...
        # Add plugin CLI options to current command
