...
# Backend selection - can be overridden with TEST_BACKEND environment variable
readonly BACKEND="${TEST_BACKEND:-both}"  # options: zsh, python, both
...
run_test_on_backend() {
    ...
    backend_cmd=$(get_backend_command "$backend")
    if [ $? -ne 0 ]; then
        return 1
    fi
    
    log_info "Testing $test_func with $backend backend"
    
    # Set the command for this test run
    CURRENT_BACKEND_CMD="$backend_cmd"
    
    # Run the test function
    "$test_func"
}

run_test_on_all_backends() {
    local test_func="$1"
    local backend_list
    ...
    case "$BACKEND" in
        ...
        python)
            ...
            ;;
        both)
            backend_list="zsh python"
            ;;
        *)
            log_error "Invalid BACKEND setting: $BACKEND"
            return 1
            ;;
    esac
    
    for backend in $backend_list; do
        run_test_on_backend "$backend" "$test_func"
    done
}

# Get appropriate API key option based on backend
get_openai_key_option() {
    local backend="$1"
    case "$backend" in
        python)
            echo "--cipher-openai-key"
            ;;
        *)
            echo "--openai-key"
            ;;
...
get_anthropic_key_option() {
    local backend="$1"
    case "$backend" in
        python)
            echo "--cipher-anthropic-key"
            ;;
        *)
            echo "--anthropic-key"
            ;;
...
assert_command_succeeds() {
    ...
    if ! validate_test_command "$command"; then
        log_error "FAIL: $description (Security validation failed)"
        (( TESTS_FAILED++ ))
        return 1
    fi
...
setup() {
    ...
    log_info "Claude MCP Init binary: $CURRENT_BACKEND_CMD"
    log_info "Root directory: $ROOT_DIR"
    log_info "Absolute binary check: $(ls -la "$CURRENT_BACKEND_CMD" 2>&1 || echo "NOT FOUND")"
    
    # Check if binary exists
    if [ ! -f "$CURRENT_BACKEND_CMD" ]; then
        log_error "Claude MCP Init binary not found: $CURRENT_BACKEND_CMD"
        log_error "Run 'make build' first"
        exit 1
    fi
    
    # Make binary executable
    chmod +x "$CURRENT_BACKEND_CMD"
...
test_version_flag() {
    ...
    assert_command_succeeds "$CURRENT_BACKEND_CMD --version" "Version flag should work"
}

test_help_flag() {
    log_info "Testing --help flag..."
    assert_command_succeeds "$CURRENT_BACKEND_CMD --help" "Help flag should work"
}

test_shell_flag() {
    log_info "Testing --shell flag..."
    assert_command_succeeds "$CURRENT_BACKEND_CMD --shell" "Shell flag should work"
}

test_invalid_flag() {
    log_info "Testing invalid flag..."
    assert_command_fails "$CURRENT_BACKEND_CMD --invalid-flag" "Invalid flag should fail"
}

test_no_arguments() {
    log_info "Testing no arguments..."
    assert_command_fails "$CURRENT_BACKEND_CMD" "No arguments should fail"
}

test_invalid_project_name() {
    log_info "Testing invalid project name..."
    assert_command_fails "$CURRENT_BACKEND_CMD '!!!invalid'" "Invalid project name should fail"
}
...
test_basic_project_creation() {
    ...
    # Create project
    assert_command_succeeds "$CURRENT_BACKEND_CMD $project_name" "Basic project creation should succeed"
...
test_project_with_language() {
    ...
    assert_command_succeeds "$CURRENT_BACKEND_CMD $project_name $language" "Project with Python language should succeed"
...
test_invalid_language() {
    ...
    assert_command_succeeds "$CURRENT_BACKEND_CMD $project_name $language" "Project with invalid language should succeed with fallback"
...
test_all_supported_languages() {
    ...
    for lang in "${languages[@]}"; do
        ...
        assert_command_succeeds "$CURRENT_BACKEND_CMD $project_name $lang" "Project with $lang should succeed"
...
test_unsupported_languages() {
    ...
    for lang in "${unsupported_langs[@]}"; do
        ...
        assert_command_succeeds "$CURRENT_BACKEND_CMD $project_name $lang" "Project with unsupported $lang should succeed with fallback"
...
test_permission_checks() {
    ...
    # Create project
    assert_command_succeeds "$CURRENT_BACKEND_CMD $project_name" "Project creation should succeed"
...
test_in_place_mode() {
    ...
    assert_command_succeeds "$CURRENT_BACKEND_CMD -n $project_name typescript" "In-place mode with -n should succeed"
    ...
    assert_command_succeeds "$CURRENT_BACKEND_CMD --in-place $project_name python" "In-place mode with --in-place should succeed"
...
test_api_key_options() {
    ...
    assert_command_succeeds "$CURRENT_BACKEND_CMD --openai-key $openai_key $project_name typescript" "Project with OpenAI key should succeed"
    ...
    assert_command_succeeds "$CURRENT_BACKEND_CMD --openai-key $openai_key --anthropic-key $anthropic_key $project_name2 python" "Project with both keys should succeed"
    ...
    assert_command_succeeds "$CURRENT_BACKEND_CMD --anthropic-key $anthropic_key $project_name3 rust" "Project with Anthropic key only should succeed"
...
test_mcp_json_generation() {
    ...
    assert_command_succeeds "$CURRENT_BACKEND_CMD $project_name typescript" "Project creation should succeed"
...
test_embedding_providers() {
    ...
    local openai_key="$(get_test_openai_key)"
    local embed_key="$(get_test_embed_key)"
    
    assert_command_succeeds "$CURRENT_BACKEND_CMD --openai-key $openai_key --cipher-embedding openai --cipher-embedding-key $embed_key $project_name1 typescript" "Project with OpenAI embedding should succeed"
    ...
    assert_file_contains "$project_name1/memAgent/cipher.yml" "apiKey: \\$OPENAI_API_KEY" "Cipher should use OpenAI API key environment variable"
    ...
    local voyage_key="$(get_test_voyage_key)"
    
    assert_command_succeeds "$CURRENT_BACKEND_CMD --openai-key $openai_key --cipher-embedding voyage --cipher-embedding-key $voyage_key $project_name2 python" "Project with Voyage embedding should succeed"
    assert_file_contains "$project_name2/memAgent/cipher.yml" "type: voyage" "Cipher should use Voyage embeddings"
    assert_file_contains "$project_name2/memAgent/cipher.yml" "apiKey: \$VOYAGE_API_KEY" "Cipher should use Voyage API key environment variable"
    assert_file_contains "$project_name2/memAgent/cipher.yml" "dimensions: 1024" "Cipher should use Voyage dimensions"
    assert_file_contains "$project_name2/memAgent/cipher.yml" "model: voyage-3-large" "Cipher should use Voyage model"
    
    cd "$TEST_DIR"
    
    # Test Gemini embedding provider
    local project_name3="test-embed-gemini"
    local gemini_key="$(get_test_gemini_key)"
    
    assert_command_succeeds "$CURRENT_BACKEND_CMD --anthropic-key $(get_test_anthropic_key) --cipher-embedding gemini --cipher-embedding-key $gemini_key $project_name3 rust" "Project with Gemini embedding should succeed"
    assert_file_contains "$project_name3/memAgent/cipher.yml" "type: gemini" "Cipher should use Gemini embeddings"
    assert_file_contains "$project_name3/memAgent/cipher.yml" "apiKey: \$GEMINI_API_KEY" "Cipher should use Gemini API key environment variable"
    ...
    assert_command_succeeds "$CURRENT_BACKEND_CMD --openai-key $openai_key --cipher-embedding ollama --cipher-embedding-key $ollama_key $project_name4 go" "Project with Ollama embedding should succeed"
...
test_embedding_priority() {
    ...
    local openai_key="$(get_test_openai_key)"
    local embed_openai="$(get_test_embed_key)"
    local embed_voyage="$(get_test_voyage_key)"
    
    assert_command_succeeds "$CURRENT_BACKEND_CMD --openai-key $openai_key --cipher-embedding openai --cipher-embedding-key $embed_openai $project_name typescript" "Project with OpenAI embedding should succeed"
    assert_file_contains "$project_name/memAgent/cipher.yml" "type: openai" "Should prioritize OpenAI embedding"
    assert_file_contains "$project_name/memAgent/cipher.yml" "apiKey: \\$OPENAI_API_KEY" "Should use OpenAI embedding key environment variable"
...
test_no_embedding_fallback() {
    ...
    assert_command_succeeds "$CURRENT_BACKEND_CMD --anthropic-key $anthropic_key --openai-key $openai_key $project_name python" "Project without embedding should succeed"
    ...
    assert_file_contains "$project_name/memAgent/cipher.yml" "apiKey: \\$OPENAI_API_KEY" "Should use OpenAI key environment variable for embedding"
    ...
    assert_command_succeeds "$CURRENT_BACKEND_CMD --openai-key $openai_key $project_name2 typescript" "Project without embedding providers should succeed"
...
run_all_tests() {
    ...
    # Individual test functions - run on all selected backends
    run_test_on_all_backends test_version_flag
    run_test_on_all_backends test_help_flag
    # Note: --shell flag is Zsh-specific, skip for Python backend
    case "$BACKEND" in
        zsh|both)
            run_test_on_backend zsh test_shell_flag
            ;;
    esac
    run_test_on_all_backends test_invalid_flag
    run_test_on_all_backends test_no_arguments
    run_test_on_all_backends test_invalid_project_name
    run_test_on_all_backends test_basic_project_creation
    run_test_on_all_backends test_project_with_language
    run_test_on_all_backends test_invalid_language
    run_test_on_all_backends test_all_supported_languages
    run_test_on_all_backends test_unsupported_languages
    run_test_on_all_backends test_permission_checks
    run_test_on_all_backends test_in_place_mode
    run_test_on_all_backends test_in_place_with_existing_dirs
    run_test_on_all_backends test_api_key_options
    run_test_on_all_backends test_mcp_json_generation
    # test_embedding_providers
    # test_embedding_priority
    # test_no_embedding_fallback
